...
class SimpleFeaturePyramid1(Backbone):
    """
...
class TimmViTWrapper(Backbone):
    def __init__(self, 
        ...
        if pretrained:
            ...
            # 保存原始位置编码
            self.register_buffer('original_pos_embed', state_dict['pos_embed'])
            del state_dict['pos_embed']  # 避免加载到ViT中
    ...
    # def _adapt_pos_embed(self, target_size):
    #     """动态调整位置编码到目标尺寸"""
    #     pos_embed = self.original_pos_embed
    #     cls_pos = pos_embed[:, :1]
    ...
    def _adapt_pos_embed(self, target_size):
        """动态调整位置编码到目标尺寸（支持矩形分块）"""
        pos_embed = self.original_pos_embed
        cls_pos = pos_embed[:, :1]
        patch_pos = pos_embed[:, 1:]
        
        # 将位置编码转换为空间格式 [1, h, w, dim]
        patch_pos = patch_pos.reshape(1, 14, 14, -1)
        
        # 插值到目标尺寸 (H_patch, W_patch)
        H_target, W_target = target_size
        patch_pos = F.interpolate(
            patch_pos.permute(0, 3, 1, 2),  # [1, dim, 14, 14]
            size=(H_target, W_target),
            mode='bicubic',
            align_corners=False
        ).permute(0, 2, 3, 1)  # [1, H_target, W_target, dim]
        
        # 展平并添加CLS token
        patch_pos = patch_pos.flatten(1, 2)  # [1, H*W, dim]
        return torch.cat([cls_pos, patch_pos], dim=1)
    ...
    #     pos_embed = self._adapt_pos_embed(grid_size).to(x.device)
    #     x = x + pos_embed
    ...
    #     return {'last_feat': x[:, 1:]}  # 移除cls token
    ...
    #     # 动态调整位置编码
    #     pos_embed = self._adapt_pos_embed((H_patch, W_patch)).to(x.device)
    #     x = x + pos_embed
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        # print(f"\n[DEBUG] 输入尺寸: {H}x{W} (batch={B})")

        # 动态填充到patch整数倍
        pad_w = (self.patch_size - W % self.patch_size) % self.patch_size
        pad_h = (self.patch_size - H % self.patch_size) % self.patch_size
        x = F.pad(x, (0, pad_w, 0, pad_h))
        # print(f"[DEBUG] 填充后尺寸: {x.shape[2]}x{x.shape[3]} (pad_h={pad_h}, pad_w={pad_w})")

        # 计算分块网格
        H_patch = (H + pad_h) // self.patch_size
        W_patch = (W + pad_w) // self.patch_size
        # print(f"[DEBUG] 分块网格: {H_patch}x{W_patch} (总块数={H_patch*W_patch})")

        # 提取patch特征
        x = self.vit.patch_embed(x)  # [B, N, D]
        # print(f"[DEBUG] 分块后特征尺寸: {x.shape}")

        # 添加CLS token
        cls_token = self.vit.cls_token.expand(B, -1, -1)
        x = torch.cat([cls_token, x], dim=1)
        # print(f"[DEBUG] 添加CLS后特征尺寸: {x.shape}")
        ...
        for blk in self.vit.blocks:
            x = blk(x)
        x = self.vit.norm(x)
        # print(f"[DEBUG] Transformer输出尺寸: {x.shape}")

        # 关键修复点：从原始张量获取维度信息
        B, seq_len, D = x.shape  # [B, N+1, D]
        # print(f"[DEBUG] 最终特征维度: B={B}, seq_len={seq_len}, D={D}")

        # 移除CLS token并转换
        features = x[:, 1:, :]  # [B, H_patch*W_patch, D]
        # print(f"[DEBUG] 移除CLS后特征尺寸: {features.shape} (应= [{B}, {H_patch*W_patch}, {D}])")

        # Reshape为空间格式
        try:
            features = features.permute(0, 2, 1).reshape(B, D, H_patch, W_patch)
            # print(f"[DEBUG] 空间特征尺寸: {features.shape} (应= [{B}, {D}, {H_patch}, {W_patch}])")
        except RuntimeError as e:
            raise RuntimeError(
                f"Reshape失败! 输入: {features.shape} 目标: [{B}, {D}, {H_patch}, {W_patch}]\n"
                f"计算分块数: {H_patch}x{W_patch}={H_patch*W_patch} vs 实际分块数: {features.shape[1]}"
            ) from e

        return {'last_feat': features}
...
class SimpleFeaturePyramid(Backbone):
    def __init__(self, 
                 ViT,
                 in_feature='last_feat',  # 新增参数
                 out_channels=256,
                 scale_factors=[4.0, 2.0, 1.0, 0.5],
                 norm="LN",
                 square_pad=0):
        super().__init__()
        self.in_feature = in_feature  # 保存特征键
        ...
        for scale in scale_factors:
            ...
            if scale == 4.0:
                ...
                layers += [nn.ConvTranspose2d(dim, dim//2, 2, 2)]
                out_dim = dim//2
            elif scale == 1.0:
                out_dim = dim
            elif scale == 0.5:
...
@BACKBONE_REGISTRY.register()
class ViTCOMMON(SimpleFeaturePyramid, Backbone):
    def __init__(self, cfg, input_shape):
        # 从配置解析参数
        vit_cfg = cfg.MODEL.ViTCOMMON
        ...
        super().__init__(
            ...
            norm=vit_cfg.NORM,
            square_pad=vit_cfg.SQUARE_PAD
        )

        # 注册输出特征
        self._out_features = vit_cfg.OUT_FEATURES
        base_stride = vit_backbone.output_shape()['last_feat'].stride
...
# 打印其他层的形状
    for key in out.keys():
        print(f"Shape of '{key}':", out[key].shape)
